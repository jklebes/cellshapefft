function Msft = inertia_matp_sigma(Param,SFT, tile_slopes)
%% Definition de M
...
for im = 1:size(SFT,1)
    ...
    reshaped = reshape(th_spec,[Param.subwindow_size*Param.subwindow_size ,1]);  % reshape the spectrum to order
    [~,index]= sort(reshaped,'descend');                     % sort in intensity and keep indexes
    nbel = Param.propor * Param.subwindow_size*Param.subwindow_size;             % nbel number of points determined by Param.propor

    if nbel > Param.subwindow_size*Param.subwindow_size            % if nbel is to large, take the whole picture
        nbel = Param.subwindow_size*Param.subwindow_size;
    else
    end
    ...
    B = false(Param.subwindow_size);

    B(th_spec>0)=1;                              % put rights where th_spec = 1
    th_spec=imclose(B,se);                       % close image

    %% Deskew and destretch here
    

    %% Inertia matrix computation

    norm = sum(sum((th_spec).^2));               % computes norm
    [Yg,Xg]=meshgrid(1:(Param.subwindow_size),1:(Param.subwindow_size)); % create meshgrid of correct size
    xc = Param.subwindow_size/2;                               % position of the center
    yc = Param.subwindow_size/2;
    Xo = Xg-xc;
    Yo = Yg-yc;
    Y = (Yg-yc).^2; % Lignes
    X = (Xg-xc).^2; % Colonnes
